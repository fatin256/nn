...
# ---------------------------------------------------------
st.set_page_config(
    page_title="PyTorch Neural Network Demo",
    layout="wide",
)

st.title("Neural Network Classification")
st.write(
    """
    This app trains a simple feed-forward neural network (MLP) using **PyTorch**  
    on a synthetic 2D dataset (moons or circles) and visualises:
    - Training loss curve  
    - Decision boundary  
    - Train & test accuracy  
    """
...
with col2:
    ...
    scatter = ax_data.scatter(X_scaled[:, 0], X_scaled[:, 1], c=y)
...
if train_button:
    ...
    ax_decision.contourf(xx, yy, Z, alpha=0.4)
    scatter = ax_decision.scatter(
        X_scaled[:, 0], X_scaled[:, 1], c=y, edgecolor="k"
    )
    ax_decision.set_xlabel("x1")
    ax_decision.set_ylabel("x2")
    ax_decision.set_title("Decision Boundary")
    st.pyplot(fig_decision)

else:
    st.info("Set your parameters in the sidebar and click **Train Model** to start.")
